...
By default imagery and uploaded files will be stored at `/data/sealog-files`.  Change this by updating the `FILEPATH_ROOT` variable in the `./config/server_settings.js`.  If this path does not exist the server will try to create it.  If it cannot the server will throw an error and exit.
